...
class Command(management.BaseCommand):
    ...
    def import_geometries(self):
        ...
        lm_centroid = LayerMapping(
            GridTile, GridTile_centroid_shp, GridTile_centroid_mapping,
            transform=False, encoding='utf-8', unique='id',
        )
    ...
    def import_neighbours(self):
        ...
        neighbours_reader = csv.DictReader(open(GridTile_neighbours_csv, 'rt'), delimiter=',', quotechar='"')

        for row in neighbours_reader:
            # Obtain fields (tile has to exclude self from neighbours)
            id = row['grid_id']
            neighbours = [neighbour for neighbour in row['neighbours'].split(',') if not (id in neighbour)]
